class CategoriesController < ApplicationController
  before_action :set_category, only: [:edit, :update, :status]
  ...
  def index
    ...
    @categories = @categories.paginate(per_page: 50, page: params[:page])
  ...
  def update
    if @category.update(category_params)
      ...
      render partail: 'errors/errors', locals: { resource: @category }
    end

  end

  def status
    if @category.desactive?
      @category.active!
    else
      @category.desactive!
    end
